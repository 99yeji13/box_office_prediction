...
#2024년 3월 21일 기준으로 과거 50주간의 데이터를 뽑아오기
#주간(월~일)까지 데이터 조회 weekGb=0
#다양한 영화/상업 영화. 한국/외국 영화. 모든 상영지역.
for i in range(50):
  key = config['KEY']
  targetDt = datetime(2024,3,21) - timedelta(weeks=i)
  targetDt = targetDt.strftime('%Y%m%d') #20240321 형식으로 변환
  ...
  for movie in movies:
      ...
      if code not in result:
          #영화코드명을 key로 가지는 딕셔너리 내부에 필요한 정보들(영화코드, 제목, 관객수)을 가지는 딕셔너리를 생성하기
          result[code] = {
